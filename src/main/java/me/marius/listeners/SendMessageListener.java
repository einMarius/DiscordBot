                    ...
                    if(!e.getMember().getUser().getId().equalsIgnoreCase("235088799074484224") || e.getMember().getUser().getId().equalsIgnoreCase("252128902418268161")) {
                        if (cooldown.containsKey(e.getMember())) {
                            ...
                            if (secondsleft > 0) {
                                ...
                                Main.plugin.getMySQL().setPunkte(e.getMember().getId(), e.getMember().getUser().getName(), 0, 1, 0, 0, 0);
                            } else if (secondsleft < 0) {
                                //RANK 5
                                if(Main.plugin.getMySQL().getPunkte(e.getMember().getId()) >= 10000){
                                    Main.plugin.getMySQL().setPunkte(e.getMember().getUser().getId(), e.getMember().getUser().getName(), 1, 1, 0, 0, 0);
                                    //RANK 4
                            ...
                            if (!Main.plugin.getMySQL().userIsExisting(e.getMember().getUser().getId())) {
                                Main.plugin.getMySQL().createNewPlayer(e.getMember().getUser().getId(), e.getMember().getUser().getName(), 1, 1, 0, 0, 0);
